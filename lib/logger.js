...
objectToStr = function(obj, maxDepth, depth) {
	...
	const
		...
		alt = depth % 2,
		indent = colorConfig.object.indent(alt);
...
function writeToStream(stream, level, source, msg, data, maxDepth, cb) {
	...
	const
		...
		caption =
			`${colorConfig[level].level(level)}` +
			` ${colorConfig.date(dateToStr(now))} ` +
			`${colorConfig[level].source(source)}`,
		
		indent = colorConfig.object.indent();
...
function prepareBunyan(source, sysInfo, data, maxDepth) {
	...
	let payload = objFieldsToStr(data, maxDepth, warnings);
...
class Logger extends EventEmitter {
	 ...
	 */
	reopenFileStreams() {
		if (this.bunyans['info'])
			this.bunyans['info'].reopenFileStreams();
		if (this.bunyans['error'])
			this.bunyans['error'].reopenFileStreams();
		if (this.bunyans['debug'])
			this.bunyans['debug'].reopenFileStreams();
	}
	...
	_write(stackDepth, level, sysInfo, msg, data) {
		...
		if (this.bunyans[level]) {
			const
				outData = data || {},
				bunyanDepth = ( level == 'debug' ) ? Infinity : this.options.maxDepthFile,
				bunyanObj = prepareBunyan(source, sysInfo || {}, outData, bunyanDepth);
		...
		if (this.options.duplicateConsole && level !== 'info') {
			const
				...
				maxDepthConsole =
					( level == 'debug' && this.options.maxDepthConsole < 10 ) ?
						10 : this.options.maxDepthConsole;
			
			let outData = (data === undefined) ? [] : [].concat(data);
	...
	_expressWrite(level, req, msg, data) {
		const
			...
			reqId = req.headers['x-nginx-request-id'] || undefined,
			reqUrl = req.url,
			...
			reqReferrer = req.headers['referer'] || undefined,
